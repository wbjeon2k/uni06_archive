...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges)
{
    ...
    printf("start mm_init test\n");

    /* Call the mm package's init function */
    if (mm_init() < 0) {
    printf("failed mm_init test\n");
	malloc_error(tracenum, 0, "mm_init failed.");
	return 0;
    }

    printf("passed mm_init test\n");
	...
	size = trace->ops[i].size;
        ...
        printf("start allocation test\n");

	    /* Call the student's malloc */
	    if ((p = mm_malloc(size)) == NULL) {
        printf("failed mm_malloc test\n");
		malloc_error(tracenum, i, "mm_malloc failed.");
		return 0;
	    }

        printf("passed allocation test\n");
	    ...
	    printf("start mm_realloc test\n");
	    if ((newp = mm_realloc(oldp, size)) == NULL) {
        printf("failed mm_realloc test\n");
		malloc_error(tracenum, i, "mm_realloc failed.");
		return 0;
	    }

        printf("passed reallocation test phase1\n");
	    ...
	    printf("passed reallocation test phase2\n");
	    ...
	    printf("start mm_free test\n");
	    mm_free(p);
	    printf("passed mm_free test\n");
	    break;
